...
import { useTranslations, useLocale } from "next-intl"; // إضافة useLocale
import axiosInstance from "@/lib/axiosInstance";
import { Loader2, MessageSquare, Star, ChevronLeft, ChevronRight, Calendar } from "lucide-react";
    ...
    <div className="w-full px-2 md:px-4 py-4" dir={isRTL ? "rtl" : "ltr"}>
      ...
      {reviews.length === 0 ? (
        ...
        <>
            ...
            <table className="w-full border-separate border-spacing-y-4">
              <thead>
                <tr className="text-gray-400 text-sm uppercase tracking-wider">
                  ...
                  <th className={`px-6 py-2 font-medium ${isRTL ? "text-right" : "text-left"}`}>{t("colFeedback")}</th>
                  <th className={`px-6 py-2 font-medium ${isRTL ? "text-right" : "text-right"}`}>
                    {t("colDate")}
                  </th>                </tr>
              </thead>
              <tbody>
                {reviews.map((r, index) => (
                  <tr key={index} className="bg-white shadow-sm hover:shadow-md transition-shadow group">
                    <td className={`px-6 py-5 border-y border-gray-50 ${isRTL ? "rounded-r-2xl border-r" : "rounded-l-2xl border-l"}`}>
                      <div className="flex items-center gap-4">
                        <FacilityAvatar facility={r.facility} locale={locale} />
                        <div>
                          <p className="font-bold text-gray-800 text-sm">
                            {r.facility?.name?.[locale] || r.facility?.name?.en || "Facility"}
                          </p>
                          <p className="text-[10px] text-gray-400 uppercase tracking-tight">{t("verifiedStay")}</p>
                        </div>
                      </div>
                    </td>
                    <td className="px-6 py-5 border-y border-gray-50">
                      <div className="flex flex-col gap-1.5">
                        ...
                        <p className={`text-gray-600 text-sm leading-relaxed max-w-md italic ${isRTL ? "text-right" : "text-left"}`}>
                          &quot;{r.comment || t("noComment")}&quot;
                        </p>
                      </div>
                    </td>
                    <td className={`px-6 py-5 border-y border-gray-50 ${isRTL ? "rounded-l-2xl border-l text-left" : "rounded-r-2xl border-r text-right"}`}>
                      <DateDisplay date={r.createdAt} locale={locale} />
                    </td>
                  </tr>
                ))}
              </tbody>
            ...
            {reviews.map((r, index) => (
              <div key={index} className="bg-white p-5 rounded-2xl shadow-sm border border-gray-50 flex flex-col gap-4">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div className={isRTL ? "text-right" : "text-left"}>
                      <p className="font-bold text-gray-800 text-sm">{r.facility?.name?.[locale] || r.facility?.name?.en}</p>
                      <DateDisplay date={r.createdAt} locale={locale} />
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <p className={`text-gray-600 text-sm leading-relaxed italic bg-gray-50 p-3 rounded-xl border border-gray-100/50 ${isRTL ? "text-right" : "text-left"}`}>
                    &quot;{r.comment || t("noComment")}&quot;
                  </p>
                </div>
              </div>
          ...
          {totalPages > 1 && (
            <div className="flex justify-center items-center gap-2 md:gap-4 mt-10" dir="ltr"> {/* Pagination usually stays LTR for numbers */}
              <button
                disabled={page === 1}
              ...
              <div className="flex gap-1.5 md:gap-2">
                {Array.from({ length: totalPages }, (_, i) => i + 1).map((num) => (
                  <button
                    key={num}
                    onClick={() => setPage(num)}
                    className={`w-9 h-9 md:w-10 md:h-10 rounded-xl text-xs md:text-sm font-bold transition-all ${page === num
...
const DateDisplay = ({ date, locale }: { date: string; locale: string }) => (
  <div className={`flex items-center gap-2 text-gray-400 text-[11px] md:text-xs font-medium ${locale === 'ar' ? 'md:justify-start' : 'md:justify-end'}`}>
    <Calendar size={13} />
    {new Date(date).toLocaleDateString(locale === 'ar' ? 'ar-EG' : 'en-GB', {
      day: "numeric",
      month: "short",
      year: "numeric",
    })}
...
const FacilityAvatar = ({ facility, locale }: { facility: any, locale: string }) => (
  <div className="relative w-10 h-10 md:w-12 md:h-12 rounded-xl overflow-hidden bg-gray-100 flex-shrink-0">
    {facility?.cover ? (
      ...
      <div className="w-full h-full flex items-center justify-center text-[#0E766E] font-bold bg-teal-50 text-sm md:text-base">
        {facility?.name?.[locale]?.charAt(0) || facility?.name?.en?.charAt(0) || "F"}
      </div>
    )}
  </div>
);